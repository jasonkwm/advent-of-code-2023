...
function checkAdjacent(inputArray, col, row, maxColLen, maxRowLen) {
    ignores = "0123456789.";
    for (let i = -1; i < 2; i++) {
        for (let j = -1; j < 2; j++) {
    ...
    for (let i = 1; i < maxColLen; i++) {
        ...
        for (let j = 0; j < maxRowLen; j++) {
            if (numbers.includes(inputArray[i][j])) {
                curNumLen++;
                if (!isPart) isPart = checkAdjacent(inputArray, i, j, maxColLen, maxRowLen);
